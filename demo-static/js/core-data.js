...
function normalizeCoreCollections(d){
  ...
  (d.invoices||[]).forEach((inv,i)=>{
    ...
    if(inv.paid===undefined && String(inv.status||'').toLowerCase()!=='draft'){ inv.paid=(docType==='invoice' && String(inv.status||'').toLowerCase()==='credit')?0:(docType==='invoice'?num(inv.total||inv.amount):0); changed=true; }
...
function runMigrations(){
  const d=db();
  if(num(d._schemaVersion) >= 3){
    if(normalizeCoreCollections(d) || normalizePromotionFields(d)) save(d);
    return d;
  }
  // Schema v3: stable promotion IDs + discount normalization, core array guards,
  // invoice balance/branch patching, product/category/supplier normalization, and sales document metadata/counters.
