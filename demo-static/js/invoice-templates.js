...
(function(){
  ...
  const defaultCompany={companyName:'Axtor Trading',legalBusinessName:'Axtor Trading W.L.L',crNumber:'CR-000000-DEMO',taxNumber:'VAT-0000-DEMO',phone:'77790896',whatsapp:'77790896',email:'sales@axtortrading.com',website:'www.axtortrading.com',address:'Street 24, Al Attiyah Street, Industrial Area',city:'Doha',country:'Qatar',bankName:'Commercial Bank of Qatar',bankAccount:'0000 0000 0000',iban:'QA00 CBQA 0000 0000 0000 0000 0',paymentInstructions:'Please arrange payment by cash, card, bank transfer or approved credit terms.',footerNote:'Thank you for choosing Axtor Trading.',terms:'Goods once sold are returnable only as per company return policy. Demo invoice only.',logoData:'',stampData:'',signatureData:'',currency:'QAR',currencySymbol:'QAR '};
  const defaultInvoice={invoicePrefix:'INV-',nextInvoiceNumber:'1048',quotationPrefix:'QT-',deliveryPrefix:'DO-',creditPrefix:'CN-',returnPrefix:'RET-',defaultInvoiceTemplate:'modern-a4',defaultQuotationTemplate:'quotation',defaultReceiptTemplate:'thermal-80',defaultPrintSize:'A4',defaultLanguage:'Bilingual',showLogo:true,showTaxNumber:true,showCrNumber:true,showCustomerBalance:true,showSalesman:true,showBarcode:true,showQr:true,showPaymentTerms:true,showBankDetails:true,showSignatureBox:true,showStamp:true,showFooterNote:true,autoPrint:false,autoPreview:true,savePdfDemo:true,copyLabel:'Original',vatEnabled:true,taxRate:DEFAULT_TAX_RATE};
  ...
  const defaultCustomerPrefs={'Al Noor Garage':{template:'paint-store',priceLevel:'Wholesale',paymentTerms:'30 Days',deliveryAddress:'Industrial Area, Doha',showBalance:true,showCreditLimit:true,whatsappDefault:true,language:'Bilingual',printSize:'A4'}};
  ...
  function audit(action,before='-',after='-'){const d=appDb(); d.auditEvents=d.auditEvents||[]; d.auditEvents.unshift({user:'Demo User',action,before,after,time:new Date().toLocaleString(),device:'Browser demo / localStorage',approval:'Not required'}); saveAppDb(d)}
  ...
  function ensure(){
    const c=read(KEYS.company,defaultCompany); if(merge(c,defaultCompany)) write(KEYS.company,c);
  ...
  function sampleData(kind='invoice', customerName){
    ...
    const customer=customerName || (d.customers&&d.customers[0]&&d.customers[0].name) || (legacy.customers&&legacy.customers[0]&&legacy.customers[0].name) || 'Al Noor Garage';
    const invoice=getInvoice();
    const items=[
      {sku:'AX-2K-040',barcode:'629100001040',brand:'Starlux',category:'Automotive Paint',name:'2K Solid Paint',product:'2K Solid Paint',colorCode:'Toyota 040',shade:'Super White',formula:'Toyota 040 Super White',batchNo:'B-2406',expiryDate:'2027-06-30',unit:'LTR',qty:2,rate:95,discount:0,taxRate:DEFAULT_TAX_RATE,warehouse:'Main WH'},
      {sku:'CC-HS-1L',barcode:'629100001055',brand:'Cosmo Coating',category:'Clear Coat',name:'HS Clear Coat 1L',product:'HS Clear Coat 1L',colorCode:'Clear',shade:'High solid gloss',formula:'Clear',batchNo:'C-1120',expiryDate:'2027-05-15',unit:'PCS',qty:1,rate:55,discount:0,taxRate:DEFAULT_TAX_RATE,warehouse:'Main WH'},
      {sku:'TN-NC-5L',barcode:'629100001038',brand:'Diamond Paints',category:'Thinner',name:'NC Thinner 5L',product:'NC Thinner 5L',colorCode:'N/A',shade:'Slow thinner',formula:'N/A',batchNo:'T-8821',expiryDate:'2027-01-20',unit:'PCS',qty:2,rate:38,discount:0,taxRate:DEFAULT_TAX_RATE,warehouse:'Main WH'}
    ];
    ...
    return {type,invoiceNo:(kind==='quotation'?invoice.quotationPrefix:kind==='delivery'||kind==='delivery-invoice'?invoice.deliveryPrefix:kind==='return'?invoice.returnPrefix:invoice.invoicePrefix)+(invoice.nextInvoiceNumber||1048),date:new Date().toLocaleDateString(),time:new Date().toLocaleTimeString(),customer,customerPhone:resolveCustomerPhone({customer})||'77790000',customerTax:'CUST-TAX-8877',customerBalance:6800,creditLimit:10000,salesmanId:firstSalesman?.id||'',salesman:firstSalesman?.name||'Ahmed Al-Rashidi',cashier:'Counter User',branch:'Main Branch',vehicleRef:'Toyota Land Cruiser - job card #LC-040',colorMatchingNote:'Color matched with shade card; verify under sunlight before final spray.',thinnerRatio:'2:1 + 10% thinner',deliveryAddress:'Industrial Area, Doha, Qatar',paymentMethod:'Cash / Card / Credit',paid:grand,balance:0,subtotal:sub,discount:disc,tax,grand,total:grand,amount:grand,items};
  }
  ...
  function tableHtml(items, templateId){
    const cols=visibleColumns(templateId).filter(c=>c!=='salesman');
  ...
  function totalsBlock(data){return `<div class="inv-totals"><div><span>Subtotal</span><strong>${money(data.subtotal)}</strong></div><div><span>Discount</span><strong>${money(data.discount)}</strong></div><div><span>Tax</span><strong>${money(data.tax)}</strong></div><div class="grand"><span>Grand Total</span><strong>${money(data.grand)}</strong></div></div>`}
  ...
  function render(templateId, opts={}){
    ...
    const t=templateList.find(x=>x.id===templateId)||templateList[0];
    ...
    if(templateId==='thermal-80') return baseCss(des)+`<div class="invoice-sheet thermal"><div class="inv-head">${brandBlock(c,inv)}<p>${esc(data.branch)}<br>${esc(data.invoiceNo)} • ${esc(data.time)}</p></div><p><b>Cashier:</b> ${esc(data.cashier)}<br><b>Customer:</b> ${esc(data.customer)}</p>${tableHtml(data.items,'thermal-80')}${totalsBlock(data)}<p><b>Payment:</b> ${esc(data.paymentMethod)}<br><b>Paid:</b> ${money(data.paid)}<br><b>Balance:</b> ${money(data.balance)}</p>${inv.showQr?'<div class="qr-box" style="margin:auto">QR Demo</div>':''}${footerBlock(c,inv,resolvedSalesman,data)}<p style="text-align:center">Thank you!</p></div>`;

    const bilingual=templateId==='bilingual'?`<div class="ar"><b>فاتورة / Invoice</b><br>التاريخ / Date: ${esc(data.date)}<br>العميل / Customer: ${esc(data.customer)}<br>الإجمالي / Total: ${money(data.grand)}</div>`:'';
    ...
    return baseCss(des)+`<div class="invoice-sheet ${cls}"><div class="inv-head">${brandBlock(c,inv)}<div class="inv-title"><h1>${title}${templateId==='bilingual'?' / فاتورة':''}</h1><p><span class="copy-label">${esc(inv.copyLabel)}</span></p><p><b>${esc(data.invoiceNo)}</b><br>${esc(data.date)}</p></div></div>${bilingual}<div class="inv-box-grid"><div class="inv-box"><b>Customer / العميل</b><br>${esc(data.customer)}<br>${esc(data.deliveryAddress)}${inv.showCustomerBalance?`<br>Balance: ${money(data.customerBalance)}`:''}</div><div class="inv-box"><b>Sales Info</b><br>Branch: ${esc(data.branch)}<br>Payment: ${esc(data.paymentMethod)}<br>Template: ${esc(t.name)}</div></div>${paint}${tax}${tableHtml(data.items,templateId)}<div style="display:flex;gap:20px;align-items:flex-start;justify-content:space-between"><div>${inv.showQr?'<div class="qr-box">QR Demo</div>':''}${inv.showBarcode?'<div class="barcode-box" style="width:180px;height:42px;margin-top:8px">Barcode Demo</div>':''}</div>${totalsBlock(data)}</div>${footerBlock(c,inv,resolvedSalesman,data)}</div>`;
  }
