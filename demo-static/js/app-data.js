...
(function(){
  ...
  function savedDocumentText(inv){
    ...
    return `${no} ${meta.label} ${inv.customer||inv.customerName||''} ${inv.date||''} ${documentAmount(inv)} ${documentStatus(inv)}`.toLowerCase();
  }
  function renderInvoices(){
    ...
    body.innerHTML=rows.map(inv=>{
      ...
      return `<tr><td><strong>${safeText(no)}</strong><br><small class="text-muted">${safeText(meta.label)}</small></td><td>${safeText(inv.customer||inv.customerName||'-')}</td><td>${safeText(inv.date||'-')}</td><td>${money(documentAmount(inv))}</td><td>${badge(documentStatus(inv)||'Paid')}</td><td>${action}</td></tr>`;
  ...
  function populateCustomerPaymentSelect(data){
    ...
    sel.innerHTML=names.map(n=>`<option>${safeText(n)}</option>`).join('') || '<option>Al Noor Garage</option>';
  ...
  function renderCustomerPayments(){
    ...
    const data=db(); populateCustomerPaymentSelect(data); const customer=$('#customerPaymentCustomer')?.value || 'Al Noor Garage';
  ...
  function initPWA(){ if('serviceWorker' in navigator && location.protocol.startsWith('http')) navigator.serviceWorker.register('sw.js?v=20260617-cachefix1').then(reg=>reg.update&&reg.update()).catch(()=>{}); }
  ensure();
  ...
  document.addEventListener('DOMContentLoaded',()=>{ ensure(); $('#savedInvoicesSearch')?.addEventListener('input',renderInvoices); $('#clearSavedInvoicesSearch')?.addEventListener('click',()=>{ const i=$('#savedInvoicesSearch'); if(i){i.value=''; renderInvoices(); i.focus();} }); renderCustomers(); renderProducts(); renderInvoices(); renderCustomerPayments(); renderSupplierPayments(); initCart(); initCustomerSave(); initProductSave(); initTaxSettings(); initDataTools(); initPaymentModules(); initShortcuts(); initPWA(); });
})();
