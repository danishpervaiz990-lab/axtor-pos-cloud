...
(function(){
  ...
  const seed={
    ...
    invoices:[
      {no:'INV-1048', customer:'Walk-in Customer', customerPhone:'77790000', date:'13 Jun 2026', amount:304.50, total:304.50, subtotal:290, tax:14.50, salesmanId:'SM001', status:'Paid', items:[
        {sku:'AX-2K-101', brand:'Starlux', name:'2K Solid Paint', product:'2K Solid Paint', productName:'2K Solid Paint', colorCode:'Toyota 040', unit:'LTR', qty:2, rate:95, price:95, discount:0, taxRate:5},
        {sku:'CC-HS-1L', brand:'Cosmo Coating', name:'HS Clear Coat 1L', product:'HS Clear Coat 1L', productName:'HS Clear Coat 1L', colorCode:'Clear', unit:'PCS', qty:1, rate:55, price:55, discount:0, taxRate:5},
        {sku:'TN-NC-5L', brand:'Diamond Paints', name:'NC Thinner 5L', product:'NC Thinner 5L', productName:'NC Thinner 5L', colorCode:'N/A', unit:'PCS', qty:1, rate:45, price:45, discount:0, taxRate:5}
      ]},
  ...
  function invoiceSettings(){let s={}; try{s=JSON.parse(localStorage.getItem('invoiceSettings')||'{}')||{}}catch(e){s={}}; s.invoicePrefix=s.invoicePrefix||'INV-'; s.nextInvoiceNumber=s.nextInvoiceNumber||'1048'; return s; }
  ...
  function normalizeCartItem(x){
    ...
    return {sku:x?.sku||'', barcode:x?.barcode||'', brand:x?.brand||'', category:x?.category||'', colorCode:x?.colorCode||'', shade:x?.shade||x?.formula||'', batchNo:x?.batchNo||'', expiryDate:x?.expiryDate||'', warehouse:x?.warehouse||'', unit:x?.unit||'PCS', name:itemName, product:itemName, productName:itemName, itemName:itemName, description:itemName, qty:number(x?.qty)||1, price, rate:price, discount:number(x?.discount), taxRate:x?.taxRate!==undefined?number(x.taxRate):5};
  }
  function buildInvoiceItemsFromCart(cart){
    return (cart||[]).map(x=>{ const itemName=itemDisplayName(x); const qty=number(x.qty)||1; const price=number(x.price ?? x.rate); const discount=number(x.discount); const taxRate=x.taxRate!==undefined?number(x.taxRate):5; const taxable=Math.max(0,(qty*price)-discount); return {sku:x.sku||'', barcode:x.barcode||'', brand:x.brand||'', category:x.category||'', colorCode:x.colorCode||'', shade:x.shade||'', batchNo:x.batchNo||'', expiryDate:x.expiryDate||'', warehouse:x.warehouse||'', name:itemName, product:itemName, productName:itemName, itemName:itemName, description:itemName, unit:x.unit||'PCS', qty, rate:price, price, discount, taxRate, tax:taxable*(taxRate/100), total:taxable+(taxable*(taxRate/100))}; });
  }
  ...
  function invoicePrintData(inv){
    ...
    const items=rawItems.map(x=>{ const itemName=itemDisplayName(x); return {
      ...
      name:itemName, product:x.product||x.productName||itemName, productName:x.productName||itemName, itemName:x.itemName||itemName, description:x.description||itemName, unit:x.unit||'PCS', qty:number(x.qty)||1, rate:number(x.rate ?? x.price ?? x.unitPrice), price:number(x.price ?? x.rate ?? x.unitPrice), discount:number(x.discount), taxRate:x.taxRate!==undefined?number(x.taxRate):5, tax:x.tax, total:x.total
    }; });
    return {
  ...
  function findProduct(query){
    ...
    return list.find(p=>String(p.sku||'').toLowerCase()===q) || list.find(p=>String(p.name||'').toLowerCase()===q) || list.find(p=>`${p.sku||''} ${p.name||''} ${p.category||''}`.toLowerCase().includes(q)) || null;
  }
  function getCart(){ try{ const raw=sessionStorage.getItem(CART); return raw===null ? [] : (JSON.parse(raw)||[]); }catch(e){return[]} }
  function setCart(cart){ sessionStorage.setItem(CART,JSON.stringify(cart||[])); renderCart(); }
  function addSaleItem(query){
    ...
    if(found) found.qty=number(found.qty)+1; else cart.push({sku:p.sku,name:p.name,product:p.name,productName:p.name,itemName:p.name,description:p.name,price:number(p.price),rate:number(p.price),qty:1,discount:0,taxRate:5});
  ...
  function cartTotals(cart=getCart()){
    ...
    const tax=cart.reduce((a,x)=>{ const base=Math.max(0,(number(x.qty)*number(x.price ?? x.rate))-number(x.discount)); const rate=x.taxRate!==undefined?number(x.taxRate):5; return a+(base*(rate/100)); },0);
  ...
  function renderCart(){
    ...
    body.innerHTML=cart.map((x,i)=>{ const itemName=itemDisplayName(x); const qty=number(x.qty)||1; const price=number(x.price ?? x.rate); const disc=number(x.discount); const taxRate=x.taxRate!==undefined?number(x.taxRate):5; const taxable=Math.max(0,(qty*price)-disc); const lineTotal=taxable+(taxable*(taxRate/100)); return editing
  ...
  function saveDraftInvoice(){
    ...
    const customer=$('#newSaleCustomer')?.value||sessionStorage.getItem(EDIT_CUSTOMER_KEY)||'Walk-in Customer'; const paymentMethod=$('#newSalePaymentMethod')?.value||sessionStorage.getItem(EDIT_PAYMENT_KEY)||'Cash'; const draft={no,customer,date:new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}),amount:totals.total,total:totals.total,subtotal:totals.sub,discount:totals.discount,tax:totals.tax,salesmanId,paymentMethod,paymentType:paymentMethod,status:'Draft',draftItems:cart.map(x=>({sku:x.sku||'',name:itemDisplayName(x),product:itemDisplayName(x),productName:itemDisplayName(x),itemName:itemDisplayName(x),description:itemDisplayName(x),qty:number(x.qty),price:number(x.price),rate:number(x.price),discount:number(x.discount),taxRate:x.taxRate!==undefined?number(x.taxRate):5}))};
  ...
  function updatePaymentModalTotals(force=false){
    ...
    if(amount && (force || amount.value==='' || number(amount.value)===0 || String(method?.value||'').toLowerCase().includes('credit'))) amount.value=String(method && String(method.value).toLowerCase().includes('credit') ? 0 : total.toFixed(2));
  ...
  function initCart(){
    $$('[data-add-cart]').forEach(btn=>btn.addEventListener('click',()=>{ const key=btn.dataset.sku || btn.dataset.name; if(!addSaleItem(key)){ const name=btn.dataset.name, price=number(btn.dataset.price); let cart=getCart(); const found=cart.find(x=>x.name===name); found?found.qty++:cart.push({name,product:name,productName:name,itemName:name,description:name,price,rate:price,qty:1,discount:0,taxRate:5}); setCart(cart); toast(`${name} added to cart`); } }));
    ...
    $('#newSaleProductSearch')?.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); const input=e.currentTarget; if(addSaleItem(input.value.trim())) input.value=''; }});
  ...
  function initProductSave(){ const btn=$('#saveProductBtn') || $('#add-product .btn-brand'); if(!btn) return; btn.addEventListener('click',e=>{ e.preventDefault(); e.stopImmediatePropagation(); const pane=$('#add-product'), inputs=$$('.form-control',pane), cat=$('.form-select',pane)?.value||'General'; const name=inputs[0]?.value?.trim(); if(!name) return toast('Product name required','warning'); const stock=number(inputs[4]?.value); const row={name,sku:inputs[1]?.value?.trim()||'SKU-'+Date.now().toString().slice(-4),barcode:inputs[2]?.value?.trim()||'',category:cat,price:number(inputs[3]?.value),stock,status:stock<=0?'Out of stock':stock<10?'Low':'In stock'}; const data=db(); data.products=data.products||[]; const existing=data.products.findIndex(p=>String(p.sku).toLowerCase()===String(row.sku).toLowerCase()); if(existing>=0) data.products[existing]={...data.products[existing],...row}; else data.products.unshift(row); save(data); renderProducts(); toast('Product saved to localStorage'); }); }
  ...
  window.AxtorDemoData={db,save,reserveInvoiceNumber,syncCustomerReceivable,updateCustomerBalances,applyInvoiceStock,reverseInvoiceStock,restockInvoiceItem,safeText,money,number};
  document.addEventListener('DOMContentLoaded',()=>{ ensure(); renderCustomers(); renderProducts(); renderInvoices(); renderCustomerPayments(); renderSupplierPayments(); initCart(); initCustomerSave(); initProductSave(); initDataTools(); initPaymentModules(); initShortcuts(); initPWA(); });
})();
