...
(function(){
  ...
  function renderInvoices(){
    ...
    const rows=(db().invoices||[]).map(enrichDocument).filter(inv=>{
      ...
      return String(inv.status||'').toLowerCase()==='draft' || isSalesInvoice(inv) || inv.documentType==='delivery_note';
    });
    body.innerHTML=rows.map(inv=>{
      ...
      return `<tr><td><strong>${safeText(no)}</strong><br><small class="text-muted">${safeText(meta.label)}</small></td><td>${safeText(inv.customer||'-')}</td><td>${safeText(inv.date||'-')}</td><td>${money(inv.amount||inv.total||inv.grand)}</td><td>${badge(inv.status||'Paid')}</td><td>${action}</td></tr>`;
    }).join('') || '<tr><td colspan="6" class="text-muted">No saved documents.</td></tr>';
  }
